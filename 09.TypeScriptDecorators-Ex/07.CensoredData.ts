...
// това е дженеретик клас, който има key - стринг, стойностите могат да бъдат всякакъв вид
...
function getLatest(seconds: number) {
    return function (target: any, methodName: string, descriptor: PropertyDescriptor) {
        ...
        descriptor.value = function () {
            ...
            // филтрираме всички items и искаме да останат само тези item, които са създадени, 
            // преди по-малко от този брой секунди
...
function censorProps<T extends { [key: string]: any }>(censorService: MockCensorService<T>) {
    return function (target: any, methodName: string, descriptor: PropertyDescriptor) {
        ...
        descriptor.value = function () {
            // вземаме всички item
            const allItems = originalMethod.call(this);
            // подаваме всички items  и той цензурира резултата
...
class UsersService {
    ...
    // стойностите ги вземаме от условието, тъй като е фабрика
    // •	getUsers – should return all users created in the last 5 seconds with some of their fields censored 
    @getLatest(5)
    @censorProps(userCensorService)
    getUsers() { return this._users; }

    @log
    // •	getEmployees – should log a message that the method was called and return all employees created in the last 10 seconds with some of their fields censored 
    @getLatest(10)
